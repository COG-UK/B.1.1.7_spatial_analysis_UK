MCC_tree_extraction = function(mcc_tre, mostRecentSamplingDatum)
	{
		...
		for (i in 1:length(mcc_tre$annotations))
			{
				...
				mcc_tab[i,c("endLon","endLat")] = cbind(annotations$coordinates[[1]], annotations$coordinates[[2]])
			}
		for (i in 1:length(mcc_tre$annotations))
			{
				...
				if (length(index) > 0)
					{
						...
						mcc_tab[i,c("startLon","startLat")] = cbind(annotations$coordinates[[1]], annotations$coordinates[[2]])
					}
			}
		l = length(mcc_tab[,1]); ll = matrix(1:l,nrow=l,ncol=l)
		for (i in 1:dim(ll)[1])
			{
				for (j in 1:dim(ll)[2])
					{
